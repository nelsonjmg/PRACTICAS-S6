...
\begin{center} \textbf{TEOREMA DEL LÍMITE CENTRAL} \end{center}
...
El Teorema del Límite Central establece que sí las observaciones $X_1, X_2, X_3 , ..., X_n$, $X$ son variables aleatorias independientes e idénticamente distribuidas con una distribución de probabilidad cualquiera y en la cual cada una de ellas tenga la misma media $\mu$ y la misma varianza $\sigma^2$ (amba finitas).
...
curve(dnorm(x, 0, 1), col = "deepskyblue", 
...
hist(MediasEstand, main = "Histograma de medias estándarizadas", xlab="Valores de m medias normales estándarizadas", prob=TRUE, col="darkolivegreen3")
curve(dnorm(x, 0, 1), col = "deeppink3", lty = 2, lwd = 2,
      add = TRUE)
```

5) Un mejor gráfico que el histograma para decidir si los datos aleatorios son aproximadamente normal es el llamado gráfico de "probabilidad normal". La idea básica es graficar los cuantiles de sus datos contra los correspondientes cuantiles de la distribución normal. Los cuantiles de un conjunto de datos preferidos son la Mediana, Q1 y Q3 los más generales. El cuantil q es el valor en los datos donde $q*100%$. También el cuantil 0.25 es Q1, el cuantil 0.5 es la mediana y el cuantil 0.75 es Q3. Los cuantiles para la distribución teórica son similares, sólo cambia el número de puntos datos menores, o sea el área a la izquierda del monto especificado. Por ejemplo, la mediana parte el área por debajo de la curva de densidad en la mitad.
...
points(xvals, dnorm(xvals, 0, 1), col = "red", type = "l",
       lty = 1, lwd = 2)
# para n=5
n <- 5
simulExp(mu, m, n)
...
n <- 20
simulExp(mu, m, n)
hist(MediasEstand, main = "Medias Exp(10); n=20", 
     xlab = "m medias exp estándarizadas",
     prob = TRUE, col = "darkolivegreen3")
xvals = seq(from=-3, to=3, by=0.01)
points(xvals, dnorm(xvals, 0, 1), col = "red", type = "l",
...
La distribucion de Poisson tiene la siguiente forma:
\begin{center} $\frac{e^{-\lambda}\lambda^k}{k1}$ \end{center}

```{r}
SimultPoisson <- function(lambda=4, tm=100, n)
{
  vectMedias <<- numeric(0)
   MediasEstand <<- numeric(0)
  for (i in 1:n)
  {
    X = rpois(n, lambda)
    vectMedias[i] <<- mean(X)
    MediasEstand[i] <<- (vectMedias[i] - lambda)/(sqrt(lambda)/sqrt(n))
  }
}
par(mfrow = c(2,2))
#Para n=1
...
tm = 100
n=1 
SimultPoisson(lambda, tm, n)
hist(MediasEstand, main = "lambda=4; n=1", 
     xlab = "m medias exp estándarizadas",
     prob = TRUE, col = "red")
xvals = seq(from = -3, to = 3, by = 0.01)
points(xvals, dnorm(xvals, 0, 1), col = "red", type = "l",
...
n=10 
SimultPoisson(lambda, tm, n)
hist(MediasEstand, main = "lambda=4; n=10", 
     xlab = "m medias exp estándarizadas",
     prob = TRUE, col = "blue")
xvals = seq(from = -3, to = 3, by = 0.01)
points(xvals, dnorm(xvals, 0, 1), col = "red", type = "l",
...
n=30 
SimultPoisson(lambda, tm, n)
hist(MediasEstand, main = "lambda=4; n=30", 
     xlab = "m medias exp estándarizadas",
...
n=50
SimultPoisson(lambda, tm, n)
hist(MediasEstand, main = "lambda=4; n=50", 
     xlab = "m medias exp estándarizadas",
     prob = TRUE, col = "green")
xvals = seq(from = -3, to = 3, by = 0.01)
points(xvals, dnorm(xvals, 0, 1), col = "red", type = "l",
