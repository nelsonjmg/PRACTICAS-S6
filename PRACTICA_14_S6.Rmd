---
title: "PRACTICA_14_S6"
author: "Nelson de Jesus Magaña Godinez"
date: "5/9/2022"
output: pdf_document
---

\begin {center} \textbf{1. CÁLCULO DE PROBABILIDADES.} \end{center}
...
Después de realizar el procedimiento anterior nos mostrará un cuadro de dialogo como el de la figura de la derecha; en dicho cuadro solamente debemos especificar el número de ensayos binomiales $( n = 8)$ y la probabilidad de
éxito $( p =1/ 2)$.
...
```{r}
local({
  .Table <- data.frame(Probability=dbinom(0:8, size=8, 
  prob=0.5))
  rownames(.Table) <- 0:8 
  print(.Table)
})
...
La probabilidad de que acierte 5 o mas es del 36%
...
```
La probabilidad de que hayan menos de 4 huracanes es del 15%
...
local({
  .x <- 0:16
  plotDistr(.x, dpois(.x, lambda=6), xlab="x", 
...
```{r}
local({
  .Table <- data.frame(Probability=dhyper(0:2, m=11, n=4, 
  k=2))
  rownames(.Table) <- 0:2 
  print(.Table)
})
...
La probabilidad de no obtener un premio es del 5.6%  
La probabilidad de obtener un premio es del 42%  
La probabilidad de obtener dos premios es del 52%  

\begin{center} \textbf{2 GENERACIÓN DE MUESTRAS ALEATORIAS DE LAS DISTRIBUCIONES} \end{center}
...
```{r}
BinomialSamples <- as.data.frame(matrix(rbinom(1*100, 
  size=15, prob=0.5), ncol=100))
